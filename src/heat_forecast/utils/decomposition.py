...
def decompose_annual_seasonality(
    ...
    Dict[Any, pandas.DataFrame]
        A dictionary mapping each `unique_id` to its decomposed DataFrame with columns:
        - 'ds'       : original dates
        - 'trend'    : extracted trend component
        - 'seasonal' : (optionally smoothed) seasonal component
        - 'remainder': data minus trend minus seasonal
    """
...
def remove_annual_component( 
    ...
    -------
    pandas.DataFrame
        A copy of `df` with an additional column:
        - `target_col_deseason` : original `target_col` minus the daily seasonal component.
    """
...
def add_annual_component(
    ...
    forecast_df : pandas.DataFrame
        Hourly forecast data containing:
          - 'unique_id' : identifier matching `season_df`
          - 'ds'        : timestamp column (hourly granularity)
          - one or more numeric forecast columns to be adjusted
    season_df : pandas.DataFrame
        Daily-frequency seasonal component containing:
          - 'unique_id' : matching identifier
          - 'ds'        : timestamp column (daily granularity)
          - 'seasonal'  : seasonal value for that day
    target_cols : list of str, optional
        List of column names in `forecast_df` to which the seasonal component will be added.
        If None, all numeric columns in `forecast_df` will be adjusted.

    Returns
    -------
    pandas.DataFrame
        A new DataFrame based on `forecast_df`, with:
          - exactly the same columns as `forecast_df`
          - each column in `target_cols` increased by the corresponding daily `seasonal` value
    """
