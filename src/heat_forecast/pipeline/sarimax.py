...
class SARIMAXConfig:
    ...
    def __post_init__(self):
        ...
        # make sure the transform is a string
        if (self.transform is not None) and (not isinstance(self.transform, str)):
            raise ValueError("transform must be a string (or None).")
        # treat None as explicit "none" (no transform)
        if self.transform is None:
            self.transform = "none"
        # make sure the SARIMAX kwargs are valid, transforms iterable orders to tuples
...
class SARIMAXPipeline:
    ...
    # ------------------------------------------------------------------
    # PREPARATION OF TRAIN (TARGET + AUX) AND VALIDATION (AUX) SETS
    # ------------------------------------------------------------------
    ...
    def describe_prepared_data(self) -> Dict[str, Any]:
        ...
        # ---------- Format the data description with PyYAML ----------
        import yaml
